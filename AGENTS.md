...
- `node scripts/validate-all.mjs` (ou `npm run validate-all`) — checagem de integridade **principal**: cobre todos os arquivos por categoria + `real.json` + `banco.json`, conferindo ids e enunciados globalmente únicos, alternativas (5 únicas para inéditas/real, 4 ou 5 para banco), gabarito dentro do intervalo, letras A–E em ordem, no máximo 3 questões por estrutura e subtópico válido. `npm run regen` = `generate.mjs` + `validate-all.mjs`.
- `node scripts/validate.mjs` — checagem antiga, mais restrita (só os arquivos sintéticos por categoria); continua existindo e passando, mas `validate-all.mjs` é a documentada como primária.
...
- `src/data/questions/real.json` — questões **reais** de provas passadas do ENEM (269 questões), com `fonte.tipo: "enem"`, `fonte.ano` e `fonte.url` apontando para o PDF oficial do INEP (download.inep.gov.br) usado na transcrição. Cobertura: 2009 a 2025 (2º dia, prova regular / 1ª aplicação), incluindo 2021 (recuperado via renderização de imagem, ver abaixo). Distribuição por categoria é desigual porque reflete a frequência real de cada assunto nas provas — categorias muito cobradas (Razão e Proporção: 49, Porcentagem: 46, Números: 33) têm bem mais questões reais que as raras (Matrizes: 0, Conjuntos: 1, Matemática Financeira: 2).
...
- `src/data/questions/banco.json` — 544 questões **reais de outras bancas** (QConcursos, Ferretto ENEM/vestibulares, Fundação Sousândrade), com `fonte.tipo: "banco"` e `fonte.banca` com o nome da banca. Importado do projeto irmão `prof-thomaz-matematica-concursos` (pasta `THOMAZ CONCURSO`, arquivo `ESTRUTURA_E_BANCO_QUESTOES.md`), que gerou esse banco especificamente pra ser reaproveitado aqui. Diferente de `real.json` (só ENEM oficial): são questões de concurso/vestibular, não da prova do ENEM, então a UI rotula como "Banco (nome da banca)" em vez de "ENEM AAAA" — não conta como `totalReais`/prova oficial. Cobre as mesmas 20 categorias do app; algumas questões têm 4 alternativas em vez de 5 (comum em bancas estilo CESPE), o que é válido no schema.
- `src/lib/questions.ts` — agrega todos os arquivos acima em `todasQuestoes`
- `scripts/generate.mjs` fecha cada categoria em `META_TOTAL = 50` questões no total. Para cada categoria calcula `realLike = (questões de real.json nela) + (questões de banco.json nela)` — lê **os dois** arquivos — e gera `need = max(0, 50 − realLike)` inéditas, com no máximo `MAX_POR_TEMPLATE = 3` questões por molde (para manter variedade estrutural). Categorias cujo `realLike >= 50` geram **zero** inéditas e o `src/data/questions/<categoria>.json` correspondente é escrito como `[]`. O script também grava `src/data/questions/_resumo.json` com `real`/`banco`/`realLike`/`inedita`/`total` por categoria. A geração é determinística (PRNG semeado em `scripts/helpers.mjs`; `templates.mjs` não pode usar `Math.random()`). Rodar `node scripts/generate.mjs` de novo após adicionar mais questões reais para rebalancear automaticamente.
...
5. Adicionar ao array em `src/data/questions/real.json` seguindo o schema de `src/lib/types.ts`, depois rodar `node scripts/validate-all.mjs` (ou `npm run validate-all`), que já cobre `real.json` (além de `banco.json` e dos arquivos por categoria): ids e enunciados globalmente únicos, 5 alternativas de texto únicas, gabarito presente entre as letras, letras em ordem A-E. (O `validate.mjs` antigo continua existindo e passando, mas só cobre os arquivos sintéticos por categoria — não precisa mais de script ad-hoc para o `real.json`.)

## Progresso do aluno

Guardado no `localStorage` do navegador (`src/lib/progress.ts`), chave `enem-questoes-progresso-v1`, e — para usuários logados — sincronizado com o Supabase (tabela `progresso_questoes`, com RLS por usuário definida em `supabase/setup.sql`; `src/app/api/keep-alive/route.ts`, chamado pelo Vercel Cron, faz um SELECT periódico para evitar a pausa do projeto Supabase por inatividade). Visitante não logado fica só no `localStorage`; no login o progresso local é mesclado com o remoto. O fluxo de correção/feedback/explicação/gabarito é client-side, então o gabarito fica no bundle enviado ao navegador (não há como escondê-lo sem mover essa lógica para um servidor).
